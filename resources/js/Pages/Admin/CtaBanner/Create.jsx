...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        title: "", description: "", button_text: "", button_link: "", image: null, is_active: true,
    });
    const handleSubmit = (e) => {
        e.preventDefault();
        post("/admin/cta-banner", { forceFormData: true });
    };
    return (
        <MepLayout>
            <Head title="Add CTA Banner" />
            <div className="max-w-2xl mx-auto px-4 py-10">
                <h1 className="text-2xl font-bold text-[#1A3A5C] mb-6">Add CTA Banner</h1>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button type="submit" disabled={processing}
                        ...
                        {processing ? "Saving..." : "Save CTA Banner"}
                    </button>
                </form>
            </div>
        </MepLayout>
