...
export default function Edit({ auth, about }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(`/admin/about/${about.id}`, { forceFormData: true });
    };
        ...
        <AuthenticatedLayout user={auth.user}>
            ...
            <div className="max-w-2xl mx-auto px-4 py-10">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-[#1A3A5C]">Edit About Section</h1>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow p-8 space-y-5">
                    ...
                    <div>
                        ...
                        {about.video && !data.video && (
                            <video src={`/storage/${about.video}`} controls
                                className="w-full sm:w-72 h-40 rounded-lg mb-2 border border-gray-200 bg-black" />
                        )}
                        <input type="file" accept="video/*"
                            onChange={(e) => setData("video", e.target.files[0])}
                            className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm" />
                        <p className="text-xs text-gray-500 mt-1">Leave empty to keep the current video. MP4 recommended, up to 50MB.</p>
